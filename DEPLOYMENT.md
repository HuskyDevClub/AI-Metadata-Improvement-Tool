...
**Deploy fails with "pending deployment in progress" after the app was auto-stopped.** `databricks apps start` itself consumes a deployment slot (it boots the app using the current workspace source path), so calling `databricks apps deploy` immediately after hits Databricks' ~20-minute per-app rate limit. `deploy.sh` avoids this by skipping the explicit deploy when it had to start the app — since `databricks repos update` already synced the latest commit into the workspace path, the start picks up the new code. If you still see this error, check that the Git Folder update succeeded before the start, and re-run the workflow.
