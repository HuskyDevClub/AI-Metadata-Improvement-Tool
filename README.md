...
2. **Configure environment variables** in your Databricks workspace:
   - `SOCRATA_APP_TOKEN`: Your Socrata API token (required)
   - `AZURE_ENDPOINT`: Azure OpenAI endpoint URL (optional)
   - `AZURE_KEY`: Azure OpenAI API key (optional)
   - `AZURE_MODEL`: Azure OpenAI model deployment name (optional)

3. **Deploy using Databricks CLI**:
   ```bash
   databricks apps deploy --app-name ai-metadata-tool
   ```
