Select Tags and Keywords for this government dataset from the portal's EXISTING tag vocabulary only. Reusing the portal's established tags keeps this dataset discoverable alongside related datasets that already use them.
...
- When two selected tags express the same idea — in different words ("kids"/"children", "jobs"/"employment"), as abbreviation and full form, or as wording variants ("license"/"licensing") — keep only ONE: whichever appears highest in the list above (most popular), and drop the others.
- When one tag's meaning is contained in another's (e.g., "water" vs "water quality"), keep the broader tag only if this dataset genuinely covers more of the broad topic than the specific tag conveys; otherwise keep just the more specific tag.
- Keep the remaining tags as distinct from one another as possible: each should capture a DIFFERENT facet of the dataset (e.g., subject, geography, program, data type). If two tags overlap heavily in meaning, keep only the stronger one. No tag should be a near-duplicate of another.
- Drop any tag that only loosely relates to the data.
